    ...
    <a-flex gap="middle">
      <a-card title="存储空间" style="width: 50%">
        <div style="height: 320px; text-align: center">
          ...
          <a-progress
            type="dashboard"
            :percent="data.sizeUsageRatio ? data.sizeUsageRatio.toFixed(2) : 0"
          />
        </div>
      </a-card>
...
import { getSpaceUsageAnalyzeVoUsingPost } from '@/service/api/spaceAnalyzeController.ts'
...
const fetchData = async () => {
  const res = await getSpaceUsageAnalyzeVoUsingPost({
